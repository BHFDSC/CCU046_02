#
# NSTEMI: 1-year mortality (amongst people discharged home) 
# Key models (complete cases)
#
# K. Fleetwood
# 21 Jan 2025: Adapted from 11a
#
...
mod_5_ang_finish <- Sys.time()
mod_5_ang_time <- difftime(mod_5_ang_finish, mod_5_ang_start, units = "mins")
mod_5_ang_time

# Summarise results
mod_5_ang_out <- glmer_out(mod_5_ang, prefix = "mod5_ang_")

# 4.7a.2 angiogram + rehab -----------------------------------------------------

mod_5_ang_reh_start <- Sys.time()
mod_5_ang_reh_start
mod_5_ang_reh <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
mod_5_ang_reh_finish <- Sys.time()
mod_5_ang_reh_time <- difftime(mod_5_ang_reh_finish, mod_5_ang_reh_start, units = "mins")
mod_5_ang_reh_time

# Summarise results
mod_5_ang_reh_out <- glmer_out(mod_5_ang_reh, prefix = "mod5_ang_reh_")

# 4.7a.3 angiogram + rehab + prevention ---------------------------------------

mod_5_ang_reh_prv_start <- Sys.time()
mod_5_ang_reh_prv_start
mod_5_ang_reh_prv <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
...
mod_5_ang_reh_prv_finish <- Sys.time()
mod_5_ang_reh_prv_time <- difftime(mod_5_ang_reh_prv_finish, mod_5_ang_reh_prv_start, units = "mins")
mod_5_ang_reh_prv_time

# Summarise results
mod_5_ang_reh_prv_out <- glmer_out(mod_5_ang_reh_prv, prefix = "mod5_ang_reh_prv_")


# 4.8 Model 5 + care (binary SMI) ---------------------------------------------

mod_5_care_bin_start <- Sys.time()
mod_5_care_bin_start
mod_5_care_bin <-
  glmer(
...
mod_5_care_bin_time <- difftime(mod_5_care_bin_finish, mod_5_care_bin_start, units = "mins")
...
mod_5_care_int_time <- difftime(mod_5_care_int_finish, mod_5_care_int_start, units = "mins")
...
mod_out <- 
  mod_5_care_out %>%
  left_join(mod_5_ang_reh_prv_out) %>%
  left_join(mod_5_ang_reh_out) %>%
  left_join(mod_5_ang_out) %>%
  left_join(mod_5_out) %>%
  left_join(mod_4_out) %>%
  left_join(mod_3_out) %>%
  left_join(mod_2_out) %>%
  ...
  select(
    ...
    mod5_ang_or_fmt, mod5_ang_reh_or_fmt, mod5_ang_reh_prv_or_fmt,
    mod5_care_or_fmt,
    #
    ...
    #
    mod5_ang_or,         mod5_ang_or_low,         mod5_ang_or_upp,
    mod5_ang_reh_or,     mod5_ang_reh_or_low,     mod5_ang_reh_or_upp,
    mod5_ang_reh_prv_or, mod5_ang_reh_prv_or_low, mod5_ang_reh_prv_or_upp,
    #
...
anova_care <- 
  anova(
    ...
    mod_5_ang, mod_5_ang_reh, mod_5_ang_reh_prv, mod_5_care
  )
anova_care <- as.data.frame(anova_care)

write.csv(
  mod_out, 
  file = file.path(outcome_folder, "12a_mort_nstemi_1yr_home_mod_care_v2.csv")
)

write.csv(
  anova_care, 
  file.path(outcome_folder, "12a_mort_nstemi_1yr_home_anova_care.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "12a_mort_nstemi_1yr_home_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "12a_mort_nstemi_1yr_home_anova_int.csv")
) 