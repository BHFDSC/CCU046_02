#
# NSTEMI: Time to mortality within the first year
# Key models 
# - (complete cases on demographics, timing and co-morbidities only)
#
# K. Fleetwood
# 29 Jan 2025: Adapted from 11b
#
...
outcome_folder <- file.path(ccu046_02_folder, "processed_data_nstemi_surv")
...
mod_2 <-
  coxme(
    Surv(mort_yr_days, mort_yr) ~ smi + age_mi_scl + I(age_mi_scl^2) + 
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "14b_mort_nstemi_surv_mod1to4.csv")
)
write.csv(
  anova_1to4, 
  file.path(outcome_folder, "14b_mort_nstemi_surv_anova1to4.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "14b_mort_nstemi_surv_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "14b_mort_nstemi_surv_anova_int.csv")
) 
