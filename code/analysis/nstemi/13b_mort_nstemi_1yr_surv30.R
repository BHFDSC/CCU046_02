#
# NSTEMI: 1-year mortality (amongst people who survived 30 days): Key models 
# - (complete cases on demographics, timing and co-morbidities only)
#
# K. Fleetwood
# 22 Jan 2025: Adapted from 11b
#
...
mod_2 <-
  glmer(
    ...
    control = 
      glmerControl(
        optimizer = "bobyqa",
        optCtrl = list(maxfun = 80000, rhoend = 1e-9)
      )
  )
...
mod_4_bin_time <- difftime(mod_4_bin_finish, mod_4_bin_start, units = "mins")
mod_4_bin_time
...
mod_4_int_time <- difftime(mod_4_int_finish, mod_4_int_start, units = "mins")
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "13b_mort_nstemi_1yr_surv30_mod1to4.csv")
)
write.csv(
  anova_1to4, 
  file.path(outcome_folder, "13b_mort_nstemi_1yr_surv30_anova1to4.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "13b_mort_nstemi_1yr_surv30_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "13b_mort_nstemi_1yr_surv30_anova_int.csv")
) 
