#
# STEMI: 1-year mortality: 
# Key models (complete cases on demographics, timing, co-morbidities and care only)
#
# K. Fleetwood
# 22 Jan 2025: Adapted from 22a
#

#
# 1. Set-up -------------------------------------------------------------------
#

## required packages
library(tidyverse)
library(dplyr)
library(janitor)
library(knitr)
library(arsenal)
library(lubridate)
library(lme4)
library(lattice)

library(performance) # includes function for checking co-linearity

home_folder <- "D:\\PhotonUser\\My Files\\Home Folder\\"
ccu046_02_folder <- file.path(home_folder, "CCU046_02")
pd_folder <- file.path(ccu046_02_folder, "Processed data")
outcome_folder <- file.path(ccu046_02_folder, "processed_data_stemi_1_yr")

r_folder <- file.path(home_folder, "r")

# Additional functions to help with summarising models
source(file.path(r_folder, "model_summary_functions.R"))  

#
# 2. Load data ----------------------------------------------------------------
#

cohort <- readRDS(file = file.path(pd_folder, "01_cohort.rds"))

# Subset cohort to STEMIs only
# Factor covariates for modelling
# Select cc
ctb_lvls <- 
  c("Call to balloon less than 120 minutes", 
    "Call to balloon between 120 and 150 minutes",
    "Call to balloon greater than or equal to 150 mins",
    "Received pPCI but CTB time unknown",
    "Did not receive pPCI"
  )

rehab_lvls <- 
  c("Yes", "No", "Not indicated", "Patient declined")

cohort <- 
  cohort %>% 
  filter(
    mi_type %in% "STEMI",
    discharge_destination %in% "Home"
  ) %>%
  mutate(
    smi = 
      factor(
        smi, 
        levels = c("No SMI", "Schizophrenia", "Bipolar disorder", "Depression")
      ),
    sex = factor(sex, levels = c("Male", "Female")),
    imd_2019 = 
      factor(
        imd_2019, 
        levels = 
          c("10 (least deprived)", "1 (most deprived)", "2", "3", "4", "5", "6",
            "7", "8", "9")
      ),
    ctb_sum = factor(ctb_sum, levels = ctb_lvls),
    cardiac_rehab = factor(cardiac_rehab, levels = rehab_lvls),
    cc = !is.na(imd_2019) & !is.na(ethnic_5) &   
         !is.na(ctb_sum)  & !is.na(prev_all) & !is.na(cardiac_rehab) &
         !is.na(procode3)
  ) %>%
  filter(cc)

# Calcualte mean and SD for age
age_mi_mean <- mean(cohort$age_mi)
age_mi_sd <- sd(cohort$age_mi)

cohort <- 
  cohort %>%
  mutate(
    age_mi_scl = (age_mi - age_mi_mean)/age_mi_sd
  )

# Create binary SMI variable
cohort <- 
  cohort %>%
  mutate(
    smi_bin = 
      case_when(
        smi %in% c("Schizophrenia", "Bipolar disorder", "Depression") ~ "Mental illness",
        smi %in% "No SMI" ~ "No mental illness"
      ),
    smi_bin = factor(smi_bin, levels = c("No mental illness", "Mental illness"))
  )

# 4. Key models ---------------------------------------------------------------

# 4.1 Model 1 -----------------------------------------------------------------

# Mixed effects logistic regression model on complete cases
# Adjust for age and sex

# Takes approximately 1 minute
mod_1_start <- Sys.time()
mod_1 <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa")
  )
mod_1_finish <- Sys.time()
mod_1_time <- mod_1_finish - mod_1_start
mod_1_time

mod_1_out <- glmer_out(mod_1, prefix = "mod1_")

# 4.2 Model 2 -----------------------------------------------------------------
# Adjust for age, sex, IMD, ethnicity

# Took 5 minutes to run
mod_2_start <- Sys.time()
mod_2 <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5  + (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa")
  )
mod_2_finish <- Sys.time()
mod_2_time <- mod_2_finish - mod_2_start
mod_2_time

# Summarise results
mod_2_out <- glmer_out(mod_2, prefix = "mod2_")

# 4.3 Model 3 -----------------------------------------------------------------

# 31 FE coefficients
# 10 * 31^2 = 8410

# Takes 11 minutes to run
mod_3_start <- Sys.time()
mod_3 <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 12000))
  )
mod_3_finish <- Sys.time()
mod_3_time <- mod_3_finish - mod_3_start
mod_3_time

mod_3_out <- glmer_out(mod_3, prefix = "mod3_")

# 4.5 Model 4 -----------------------------------------------------------------

# Sociodemographics, timing, comorbidities

mod_4_start <- Sys.time()
mod_4 <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_finish <- Sys.time()
mod_4_time <- difftime(mod_4_finish, mod_4_start, units = "mins")
mod_4_time

mod_4_out <- glmer_out(mod_4, prefix = "mod4_")

# 4.6 Model 4 + care ----------------------------------------------------------

mod_4_care_start <- Sys.time()
mod_4_care <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      ctb_sum + prev_all + cardiac_rehab + 
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_care_finish <- Sys.time()
mod_4_care_time <- difftime(mod_4_care_finish, mod_4_care_start, units = "mins")
mod_4_care_time

mod_4_care_out <- glmer_out(mod_4_care, prefix = "mod4_care_")

# 4.6a Individual care variables -------------------------------

# 4.6a1 CTB (binary) ----------------------------------------------------------

mod_4_ctb_bin_start <- Sys.time()
mod_4_ctb_bin <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      ctb_bin + 
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_ctb_bin_finish <- Sys.time()
mod_4_ctb_bin_time <- difftime(mod_4_ctb_bin_finish, mod_4_ctb_bin_start, units = "mins")
mod_4_ctb_bin_time

mod_4_ctb_bin_out <- glmer_out(mod_4_ctb_bin, prefix = "mod4_ctb_bin_")

# 4.6a2 CTB -------------------------------------------------------------------

mod_4_ctb_start <- Sys.time()
mod_4_ctb <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      ctb_sum + 
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_ctb_finish <- Sys.time()
mod_4_ctb_time <- difftime(mod_4_ctb_finish, mod_4_ctb_start, units = "mins")
mod_4_ctb_time

mod_4_ctb_out <- glmer_out(mod_4_ctb, prefix = "mod4_ctb_")

# 4.6a3 CTB + rehab -----------------------------------------------------------

mod_4_ctb_reh_start <- Sys.time()
mod_4_ctb_reh <-
  glmer(
    mort_yr ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      ctb_sum + cardiac_rehab + 
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_ctb_reh_finish <- Sys.time()
mod_4_ctb_reh_time <- difftime(mod_4_ctb_reh_finish, mod_4_ctb_reh_start, units = "mins")
mod_4_ctb_reh_time

mod_4_ctb_reh_out <- glmer_out(mod_4_ctb_reh, prefix = "mod4_ctb_reh_")

# 4.7 Model 4 + care (binary SMI) ---------------------------------------------

mod_4_care_bin_start <- Sys.time()
mod_4_care_bin <-
  glmer(
    mort_yr ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      ctb_sum + prev_all + cardiac_rehab + 
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_care_bin_finish <- Sys.time()
mod_4_care_bin_time <- difftime(mod_4_care_bin_finish, mod_4_care_bin_start, units = "mins")
mod_4_care_bin_time

mod_4_care_bin_out <- glmer_out(mod_4_care_bin, prefix = "mod4_care_bin_")

# 4.7 Model 4 + care (binary SMI) + interaction -------------------------------

mod_4_care_int_start <- Sys.time()
mod_4_care_int <-
  glmer(
    mort_yr ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab + 
      comorb_crf + comorb_cevd + 
      ctb_sum + prev_all + cardiac_rehab + 
      smi_bin:period_4 +
      (1|procode3),
    family = binomial(link = "logit"),
    data = cohort,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 45000))
  )

mod_4_care_int_finish <- Sys.time()
mod_4_care_int_time <- difftime(mod_4_care_int_finish, mod_4_care_int_start, units = "mins")
mod_4_care_int_time

mod_4_care_int_out <- glmer_out(mod_4_care_int, prefix = "mod4_care_int_")

# 5. Combine results ----------------------------------------------------------

mod_out <- 
  mod_4_care_out %>%
  left_join(mod_4_ctb_reh_out) %>%
  left_join(mod_4_ctb_out) %>%
  left_join(mod_4_ctb_bin_out) %>%
  left_join(mod_4_out) %>%
  left_join(mod_3_out) %>%
  left_join(mod_2_out) %>%
  left_join(mod_1_out) %>%
  select(
    covariate, 
    mod1_or_fmt, mod2_or_fmt, mod3_or_fmt, mod4_or_fmt, 
    mod4_ctb_bin_or_fmt, mod4_ctb_or_fmt, mod4_ctb_reh_or_fmt,
    mod4_care_or_fmt,
    #
    mod1_or,             mod1_or_low,             mod1_or_upp,
    mod2_or,             mod2_or_low,             mod2_or_upp,
    mod3_or,             mod3_or_low,             mod3_or_upp,
    mod4_or,             mod4_or_low,             mod4_or_upp,
    #
    mod4_ctb_bin_or, mod4_ctb_bin_or_low, mod4_ctb_bin_or_upp,
    mod4_ctb_or,     mod4_ctb_or_low,     mod4_ctb_or_upp,
    mod4_ctb_reh_or, mod4_ctb_reh_or_low, mod4_ctb_reh_or_upp,
    #
    mod4_care_or,        mod4_care_or_low,        mod4_care_or_upp
  )

mod_out %>%
  select(
    covariate, 
    mod1_or_fmt, mod2_or_fmt, mod3_or_fmt,mod4_or_fmt, 
    mod4_ctb_bin_or_fmt, mod4_ctb_or_fmt, mod4_ctb_reh_or_fmt,
    mod4_care_or_fmt
  )

# ANOVA adding comorbidites first and care second
anova_cm_care <- 
  anova(
    mod_1, mod_2, mod_3, mod_4, 
    mod_4_ctb_bin, mod_4_ctb, mod_4_ctb_reh, mod_4_care
  )

anova_cm_care <- as.data.frame(anova_cm_care)

write.csv(
  mod_out, 
  file = file.path(outcome_folder, "22b_mort_stemi_1yr_home_mod1to4_care.csv")
)


write.csv(
  anova_cm_care, 
  file.path(outcome_folder, "22b_mort_stemi_1yr_home_anova_cm_care.csv")
)

# Combine models 4 (binary) and 4 (interaction)

dim(mod_4_care_int_out)  

mod_out_care_int <- 
  mod_4_care_int_out %>%
  left_join(mod_4_care_bin_out) %>%
  select(
    covariate, 
    mod4_care_bin_or_fmt, mod4_care_int_or_fmt,
    mod4_care_bin_or, mod4_care_bin_or_low, mod4_care_bin_or_upp,
    mod4_care_int_or, mod4_care_int_or_low, mod4_care_int_or_upp,
  )

dim(mod_out_care_int)

write.csv(
  mod_out_care_int, 
  file = file.path(outcome_folder, "22b_mort_stemi_1yr_home_mod_int.csv")
)

anova_int <- anova(mod_4_care_bin, mod_4_care_int)

write.csv(
  anova_int,
  file = file.path(outcome_folder, "22b_mort_stemi_1yr_home_anova_int.csv")
) 

